...
class DHSPG(Optimizer):
    ...
    def __init__(self, params, lr=required, lmbda=required, lmbda_amplify=1.1, hat_lmbda_coeff=10, weight_decay=None, first_momentum=0.0, second_momentum=0.99, dampening=0.0, 
                 adam=True, target_group_sparsity=0.5, tolerance_group_sparsity=0.01, partition_step=1e5, half_space_project_steps=2e5, warm_up_steps=None):
        ...
        if dampening is not None and dampening < 0.0:
        ...
        self.warm_up_steps = self.half_space_project_steps // 2 if warm_up_steps is None else warm_up_steps
        ...
        self.x_norm_mag_penal = 0.0
        self.x_norm_not_mag_penal = 0.0

        defaults = dict(lr=lr, lmbda=lmbda, lmbda_amplify=lmbda_amplify, hat_lmbda_coeff=hat_lmbda_coeff, weight_decay=weight_decay, first_momentum=first_momentum, second_momentum=second_momentum, \
                        dampening=dampening, adam=adam, num_groups_penalize_mag=0)
    ...
    def partition_groups(self):
        ...
        self.K = int(total_num_groups * self.target_group_sparsity)
        ...
        self.num_groups_penal_mag = 0
        self.num_groups_not_penal_mag = 0
        ...
        for i, group in enumerate(self.param_groups):
            if group['group_type'] >= GROUP_TYPE['multi-head-linear']:
                ...
                group['num_groups_penalize_mag'] = len(group['magnitude_penalize'])
                group['num_groups_not_penalize_mag'] = len(group['not_magnitude_penalize'])

                assert group['num_groups_penalize_mag'] + group['num_groups_not_penalize_mag'] == group['num_groups']

                self.x_norm_mag_penal += torch.sum(torch.norm(flatten_x[group['magnitude_penalize']], p=2, dim=1))
                self.x_norm_not_mag_penal += torch.sum(torch.norm(flatten_x[group['not_magnitude_penalize']], p=2, dim=1))
                self.num_groups_penal_mag += group['num_groups_penalize_mag']
                self.num_groups_not_penal_mag += group['num_groups_not_penalize_mag']

    def compute_grad_variant(self):
        for i, group in enumerate(self.param_groups):
            first_bias_correction = 1.0 - group['first_momentum'] ** self.num_steps if group['adam'] else None
            second_bias_correction = 1.0 - group['second_momentum'] ** self.num_steps if group['adam'] else None
    ...
    def reach_target_group_sparsity(self):
        if self.curr_group_sparsity <= (self.target_group_sparsity - self.tolerance_group_sparsity):
            return False
        else:
            return True
    ...
    def step(self, closure=None):
        ...
        if self.num_steps % self.partition_step == 0 and self.partitoned is False:
            self.partitoned = True
        ...
        for i, group in enumerate(self.param_groups):
            if group['group_type'] == GROUP_TYPE['no-update']:
                ...
                if group['num_groups_penalize_mag'] > 0 and not self.reach_target_group_sparsity():
                    ...
                    if self.num_steps >= self.warm_up_steps:
                        ...
                        if self.num_steps >= self.half_space_project_steps:
    ...
    def compute_norm_group_partitions(self):
        ...
        for i, group in enumerate(self.param_groups):
            if group['group_type'] >= GROUP_TYPE['multi-head-linear']:
                ...
                self.num_groups_penal_mag += group['num_groups_penalize_mag']
                self.num_groups_not_penal_mag += group['num_groups_not_penalize_mag']
        return self.x_norm_mag_penal.cpu().item(), self.x_norm_not_mag_penal.cpu().item(), self.num_groups_penal_mag, self.num_groups_not_penal_mag